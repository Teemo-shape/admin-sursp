<template>
    <div class='proList boxs addProduct'>
        ...
        <div class="addProduct-info">
            ...
            <div class="proType">
                <div class="proType-item matter" :class="{active:proType==1}" @click="changeType(1)">
                    <span class="iconfont iconxinzengshangpin-shangpin"></span>
                    <p>实物</p>
                    <img src="@img/proActive.png" v-if="proType==1" alt="">
                </div>
                <div class="proType-item coupon" :class="{active:proType==2}" @click="changeType(2)">
            ...
            <el-form ref="form" :model="form" label-width="100px" size="medium" :rules="rules">
                ...
                <el-form-item label="商品主图" prop="fileId">
                    ...
                    <span v-for="(item,index) in picFileMoreArr" class="setMainImg" >
                        <el-button type="primary" size="mini" v-if="!item.isMain" @click="setMainImg(index)">设为列表图</el-button>
                        <el-button type="primary" plain size="mini" v-else>商品列表图</el-button>
                    </span>
                </el-form-item>
                <el-form-item label="">
                ...
                <el-form-item label="售卖信息">
                    <table cellspacing="0" cellpadding="0" style="width:100%;margin:0" class="sizeTable" v-show="isShowTable">
                        <th v-for="item in specList">{{item.specName}}</th>
                    ...
                    <div class="setBatch" v-show="isShowTable">批量设置：
                        ...
                        <span v-show="!isShowBatch"><input type="text" v-model="batchNum">
                            ...
                            <span class="batchBtns" @click="isShowBatch = true">取消</span>
                        </span>
                    </div>
                    <table cellspacing="0" cellpadding="0" style="width:100%;" class="sizeTable saleInfo"  v-show="!isShowTable">
                        <tr>
                            <td style="width:15%"><span style="color:#F56C6C">*</span>市场价 <el-input v-model="saleInfoPrice.jdPrice" :disabled="isShowTable" style="display:inline;"></el-input></td>
                            <td style="width:15%"><span style="color:#F56C6C">*</span>成本价 <el-input v-model="saleInfoPrice.price" :disabled="isShowTable" style="display:inline;"></el-input></td>
                            <td style="width:15%"><span style="color:#F56C6C">*</span>兑换价 <el-input v-model="saleInfoPrice.integral" :disabled="isShowTable" style="display:inline;"></el-input></td>
                            <td style="width:15%"><span style="color:#F56C6C">*</span>可售库存 <el-input v-model="saleInfoPrice.skuNum" :disabled="isShowTable" style="display:inline;"></el-input></td>
                        </tr>
                    </table>
                </el-form-item>
...
<script>
import { deleteFile,addProduct,getItemNorm } from '@/api/api.js'
...
export default {
    ...
    methods: {
        ...
        remove1(file, fileList) {
            let fileIdaa = file.response.data.fileId
            console.log(fileList)
            console.log(file.response.data.fileId)
            // console.log(fileIdaa)
            if(!fileIdaa){
                return;
            }
            deleteFile({ fileId: fileIdaa }).then(res=>{
                for(let i = 0;i<this.picFileMoreArr.length;i++){
                    console.log(this.picFileMoreArr[i])
                ...
                if (valid) {
                    ...
                    if(this.isShowTable){
                        this.proArr.forEach(item=>{
                            ...
                            else if(!isDecimal.test(item.price)){
                                this.$message.error('成本价只能为正整数或保留两位小数!')
                                flag = true
                            }
                            ...
                            else if(!isDecimal.test(item.jdPrice)){
                                this.$message.error('市场价只能为正整数或保留两位小数!')
                                flag = true
                            }
                            ...
                            else if(!isDecimal.test(item.integral)){
                                this.$message.error('兑换价只能为正整数或保留两位小数!')
                                flag = true
                            }
                        ...
                        for(let key in this.saleInfoPrice){
                            ...
                            else if(!isDecimal.test(this.saleInfoPrice.jdPrice)){
                                this.$message.error('市场价只能为正整数或保留两位小数!')
                                flag = true
                            }
                            else if(!isDecimal.test(this.saleInfoPrice.price)){
                                this.$message.error('成本价只能为正整数或保留两位小数!')
                                flag = true
                            }
                            else if(!isDecimal.test(this.saleInfoPrice.integral)){
                                this.$message.error('兑换价只能为正整数或保留两位小数!')
                                flag = true
                            }
                            else if(!noZeroNum.test(this.saleInfoPrice.skuNum)){
                                this.$message.error('可售库存只能为整数!')
                                flag = true
                            }
                            // if(!noZeroNum.test(this.saleInfoPrice[key])){
